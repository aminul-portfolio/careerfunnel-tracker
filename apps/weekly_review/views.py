...
def weekly_review_list(request):
    ...
    return render(request, "weekly_review/weekly_review_list.html", {"reviews": reviews, "table_rows": build_weekly_review_table_rows(reviews), "summary": build_weekly_review_summary(request.user)})


@login_required
def weekly_review_detail(request, pk):
    review = get_object_or_404(WeeklyReview, pk=pk, user=request.user)
    return render(request, "weekly_review/weekly_review_detail.html", {"review": review, "variance_badge_class": get_variance_badge_class(review.application_variance), "diagnosis_badge_class": get_diagnosis_badge_class(review.diagnosis)})
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                form.add_error("week_ending", "You already have a weekly review for this week ending date. Edit the existing review instead.")
            else:
                messages.success(request, "Weekly review created successfully.")
                return redirect(review.get_absolute_url())
    else:
        form = WeeklyReviewForm()
    return render(request, "weekly_review/weekly_review_form.html", {"form": form, "page_title": "Add Weekly Review", "submit_label": "Save Weekly Review"})
...
def weekly_review_update(request, pk):
    ...
    return render(request, "weekly_review/weekly_review_form.html", {"form": form, "page_title": "Edit Weekly Review", "submit_label": "Update Weekly Review", "review": review})
