...
def daily_log_list(request):
    ...
    return render(
        request,
        "daily_log/daily_log_list.html",
        {
            "logs": logs,
            "table_rows": build_daily_log_table_rows(logs),
            "summary": build_daily_log_summary(request.user),
        },
    )
...
def daily_log_create(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                form.add_error(
                    "log_date",
                    "You already have a daily log for this date. Edit the existing log instead.",
                )
            else:
    ...
    return render(
        request,
        "daily_log/daily_log_form.html",
        {
            "form": form,
            "page_title": "Add Daily Log",
            "submit_label": "Save Daily Log",
        },
    )
