...
class JobApplicationForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        for field_name in ["pipeline_stage", "follow_up_status"]:
            self.fields[field_name].required = False

    class Meta:
        model = JobApplication
        fields = [
            "company_name", "job_title", "job_url", "location", "work_type", "salary_range",
            "source", "role_fit", "experience_level", "required_skills", "job_description",
            "date_applied", "status", "pipeline_stage", "response_date", "cv_version",
            "cover_letter_version", "is_cv_tailored", "is_cover_letter_tailored",
            "portfolio_project_included", "company_researched", "follow_up_date",
            "follow_up_status", "last_contacted_date", "next_action", "contact_name",
            "contact_email", "notes",
        ]
        widgets = {
            ...
            "required_skills": forms.Textarea(attrs={"class": "form-control", "rows": 4, "placeholder": "Python, SQL, Excel, Power BI, reporting..."}),
            "job_description": forms.Textarea(attrs={"class": "form-control", "rows": 5, "placeholder": "Paste short job description or key requirements."}),
            ...
            "next_action": forms.TextInput(attrs={"class": "form-control", "placeholder": "Example: Send follow-up / prepare interview walkthrough"}),
    ...
    def clean(self):
        ...
        if not cleaned_data.get("pipeline_stage"):
            cleaned_data["pipeline_stage"] = "job_found"
