...
| Technical implementation | Django applications and metrics; Skill Ledger/evidence alignment; Career Evidence; fail-closed LLM provider boundary; offline AI evaluation; claim-safety controls; small private-corpus RAG evaluation; bounded read-only tool-calling; authenticated AI evidence surface. |
...
| Evidence/validation status | **3,012 / 3,012 PASS as of Sprint 124B; GitHub Actions Django CI #241 PASS.** |
| Limitation answer | Portfolio-scale project, not live SaaS. AI is configuration-gated, advisory, and human-reviewed. Small private-corpus RAG is not enterprise RAG; tools are bounded/read-only; no autonomous agents, auto-apply, Gmail/Calendar automation, production vector DB, or production AI reliability claim. |
...
- **Action:** Modelled applications and metrics in Django; added data-quality checks, exports, Skill Ledger/evidence alignment, a fail-closed LLM provider boundary, offline evaluation, small private-corpus RAG evaluation, bounded read-only tool-calling, claim-safety controls, and authenticated evidence surfaces.
...
## CareerFunnel AI interview boundaries

- **RAG:** small private corpus, cached embeddings, deterministic similarity retrieval; not enterprise RAG or a production vector database.
- **Tool-calling:** bounded, read-only, closed registry, budget constrained; not autonomous agents.
- **Live provider:** one synthetic, explicitly gated, cost-bounded canary with `cap=1`, `retries=0`; integration evidence only.
- **Evaluation:** 166 offline cases across AI quality, RAG, and bounded tool-assistant suites; not production-user metrics.
- **Workflow:** advisory, evidence-grounded, claim-safe, human-reviewed, manual action/save.
- **Reliability:** no production AI reliability, uptime, scale, or customer-performance claim.
...
- "CareerFunnel is an autonomous AI agent" or "CareerFunnel auto-applies for jobs."
- "CareerFunnel uses enterprise RAG", "a production vector database", or has production AI reliability.
- "CareerFunnel has Gmail/Calendar automation, scraping, or OAuth-driven inbox automation."
